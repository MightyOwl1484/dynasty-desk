...
const league = { id: 'league-1', phase: 'open', phaseVersion: 2, matchWeek: 3 };
...
test('locking a week locks submitted actions and creates deterministic fallback actions', () => {
  const result = lockMatchWeek(league, [action, createClubActions({ leagueId: 'league-1', clubId: 'ivr', managerId: 'manager-2' })], '2026-09-23T12:00:00Z', { actorId: 'commissioner-1' });
  ...
  assert.equal(result.auditEvents[0].type, 'week_locked');
  assert.equal(result.auditEvents[0].actorId, 'commissioner-1');
});
...
test('resolution and publication are explicit workflow steps', () => {
  ...
  const published = publishMatchWeek(resolving, [{ fixtureId: 'f1', homeGoals: 1, awayGoals: 0 }], '2026-09-23T18:00:00Z', { actorId: 'commissioner-1' });
  assert.equal(published.league.phase, 'published');
  assert.equal(published.results[0].publishedAt, '2026-09-23T18:00:00Z');
  assert.equal(published.auditEvents[0].type, 'week_published');
});
